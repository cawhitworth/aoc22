...
use std::{
    fs::{File, self},
    io::{BufRead, BufReader}, fmt::Write,
};
...
impl HeightMap {
    ...
    fn visible(&self, x: usize, y: usize) -> bool {
        ...
        // From left
        let index_left = (1..=x).
            fold(0, |acc, i| if self.height(i, y) > self.height(acc, y) { i } else { acc });
        if index_left == x {
            return true;
        }
        // From right
        let index_right = (x+1..self.w).
            fold(x, |acc, i| if self.height(i, y) >= self.height(acc, y) { i } else { acc });
        if index_right == x {
            return true;
        }
        ...
        // From top
        let index_top = (1..=y).
            fold(0, |acc, i| if self.height(x, i) > self.height(x, acc) { i } else { acc });
        if index_top == y {
            return true;
        }
        // From bottom
        let index_bottom = (y+1..self.h).
            fold(y, |acc, i| if self.height(x, i) >= self.height(x, acc) { i } else { acc });
        if index_bottom == y {
            return true;
        }
        false
    ...
    where I: Iterator<Item = (usize, usize)> {
        let (x,y) = iter.max_by(|(x1,y1),(x2,y2)| self.height(*x1,*y1).cmp(&self.height(*x2,*y2))).unwrap();
        (x,y,self.height(x, y))
    }
